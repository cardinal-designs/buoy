...
// Prevent elastic scroll immediately upon touchmove
function preventElasticScroll(event) {
  event.preventDefault();
}
...
function onTouchStart(event, supplementSideDrawer) {
  ...
  if (isScrolledToTop(supplementSideDrawer)) {
    ...
    // Prevent elastic scroll on iOS immediately upon touchstart
    supplementSideDrawer.addEventListener('touchmove', preventElasticScroll, { passive: false });
  }
}
...
function onTouchEnd(event, supplementSideDrawer, closeDrawerButton) {
  ...
  if (isScrolledToTop(supplementSideDrawer)) {
    ...
    setTimeout(() => {
      ...
      // Remove the elastic scroll prevention after touch end
      supplementSideDrawer.removeEventListener('touchmove', preventElasticScroll);
    }, 1500);
  }
}
